...
export default function CalendarScreen() {
  ...
  // ============================================================
  // TODO(eliminar líneas 14-15): Datos mock. Reemplazar con backend
  // ============================================================
  ...
  //     .then(res => {
  //       setMatchData(res.data.nextMatch);
  ...
  const handleConfirmAttendance = (willAttend: boolean) => {
    // ============================================================
    // TODO(eliminar líneas 36-43): Mock alert. Reemplazar con backend
    // ============================================================
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* Confirmar Asistencia - Prioritario */}
        {matchData && matchData.requiresConfirmation && !attendanceConfirmed && (
          <View style={[styles.priorityCard, { backgroundColor: colors.greenHouse['100'], borderColor: colors.greenHouse['600'] }]}>
            ...
            <Text style={[styles.priorityTitle, { color: colors.greenHouse['900'] }]}>
              ¡Confirma tu asistencia!
            </Text>
            <Text style={[styles.prioritySubtitle, { color: colors.greenHouse['800'] }]}>
              Partido del {matchData.day}
            </Text>
            ...
            >
              <View style={styles.eventHeader}>
                ...
                <View style={styles.eventInfo}>
                  ...
                  {event.time && (
                    <Text style={[styles.eventTime, { color: colors.greenHouse['700'] }]}>
                      🕐 {event.time}
                    </Text>
                  )}
                </View>
              ...
              <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
                {selectedEventModal && (
                  <>
                    ...
                    <Text style={[styles.modalSubtitle, { color: colors.greenHouse['700'] }]}>
                      ⚽ {selectedEventModal.event}
                    </Text>

                    <View style={styles.modalDetails}>
                      <Text style={[styles.modalDetailRow, { color: colors.foreground }]}>
                        🕐 {selectedEventModal.time}
                      </Text>
                      <Text style={[styles.modalDetailRow, { color: colors.foreground }]}>
                        🆚 {selectedEventModal.versus}
                      </Text>
                      <Text style={[styles.modalDetailRow, { color: colors.foreground }]}>
                      ...
                      {selectedEventModal.details && (
                        <>
                          ...
                          <Text style={[styles.modalDetailRow, { color: colors.mutedForeground }]}>
                            {selectedEventModal.details.uniform}
                          </Text>
                          <Text style={[styles.modalDetailRow, { color: colors.mutedForeground }]}>
                            {selectedEventModal.details.arrivalTime}
                          </Text>
                          <Text style={[styles.modalDetailRow, { color: colors.mutedForeground }]}>
                            {selectedEventModal.details.parking}
                          </Text>
                          <Text style={[styles.modalDetailRow, { color: colors.mutedForeground }]}>
                            {selectedEventModal.details.contact}
                          </Text>
                          <Text style={[styles.modalDetailRow, { color: colors.mutedForeground }]}>
                            {selectedEventModal.details.weather}
                          </Text>
                        </>
                      )}
                    </View>
